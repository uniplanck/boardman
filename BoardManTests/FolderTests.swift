...
final class FolderTests {
    ...
    func deepCopyObject() throws {
        let savedFolder = BoardManFolder()
        savedFolder.index = 100
        savedFolder.title = "saved realm folder"

        let savedSnippet = BoardManSnippet()
    ...
    func createFolder() {
        ...
        let folder2 = BoardManFolder.create()
    ...
    func mergeFolder() throws {
        let realm = try! Realm()
        #expect(realm.objects(BoardManFolder.self).isEmpty)
        ...
        folder.merge()
        #expect(folder.realm == nil)
        #expect(realm.objects(BoardManFolder.self).count == 1)

        let savedFolder = try #require(realm.object(ofType: BoardManFolder.self, forPrimaryKey: folder.identifier))
    ...
    func removeFolder() {
        ...
        #expect(realm.objects(BoardManFolder.self).count == 1)
        #expect(realm.objects(BoardManSnippet.self).count == 1)
        ...
        #expect(realm.objects(BoardManFolder.self).isEmpty)
        #expect(realm.objects(BoardManSnippet.self).isEmpty)
    }
    ...
    func rearrangeFolderIndex() {
        ...
        BoardManFolder.rearrangesIndex([copyFolder, copyFolder2])
