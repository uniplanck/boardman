...
> 릴리스 상태: 현재 정식 제품 라인은 **v0.2.1**입니다. 이전 실험 라인 이후 의도적으로 버전을 재설정했으며, 기존 v0.1.x / v1.2.x 태그는 재사용하지 않고 보존합니다. `main` 브랜치는 계속 개발 중이고 이 번역은 영어 README보다 늦을 수 있습니다.
...
- [Board-Man v0.2.1 다운로드](https://github.com/uniplanck/boardman/releases/tag/v0.2.1)
- macOS 앱 아카이브: `Board-Man-v0.2.1.zip`

## 설치 및 첫 실행

1. 릴리스 페이지에서 `Board-Man-v0.2.1.zip`을 다운로드합니다.
2. 아카이브의 압축을 풉니다.
...
문구는 자주 붙여넣는 템플릿, URL, 명령어 및 기타 콘텐츠를 위한 재사용 가능한 텍스트 항목입니다.
...
- 자주 재사용하는 텍스트
- 반복해서 실행하는 명령어
- 작업 흐름의 중심이 되는 자료나 문구
- 문구나 자동화로 바꿀 만한 복사/붙여넣기 패턴
