...
> 現在の正式な製品ラインは **v0.2.0** です。旧実験版から意図的にバージョンをリセットした後の正式ラインです。過去のv0.1.x / v1.2.xタグは再利用せず履歴として保持し、`main`ブランチにはタグ付きリリースへ未収録の変更が含まれる場合があります。
...
[Board-Man v0.2.0](https://github.com/uniplanck/boardman/releases/tag/v0.2.0)から`Board-Man-v0.2.0.zip`をダウンロードして展開し、`Board-Man.app`を`/Applications`へ移動してください。
