...
> 現在GitHubで公開済みの正式版は **v0.2.1** です。`main`の開発版は **v0.2.2（build 4）** として準備しており、v0.2.1公開後のクリップボード改行補正を含みます。`v0.2.2`タグと配布ZIPは、正式にリリースするまでは存在しません。過去のv0.1.x / v1.2.xタグは再利用せず履歴として保持します。
...
[Board-Man v0.2.1](https://github.com/uniplanck/boardman/releases/tag/v0.2.1)から`Board-Man-v0.2.1.zip`をダウンロードして展開し、`Board-Man.app`を`/Applications`へ移動してください。
