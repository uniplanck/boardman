//
//  BoardManPreferencesWindowController.swift
//
//  Clipy
...
final class BoardManPreferencesWindowController: NSWindowController {

    // MARK: - Properties
    static let sharedController = BoardManPreferencesWindowController(windowNibName: "BoardManPreferencesWindowController")
    ...
    // ViewController
    private let viewController = [NSViewController(nibName: "BoardManGeneralPreferenceViewController", bundle: nil),
                                  NSViewController(nibName: "BoardManHistoryPreferenceViewController", bundle: nil),
                                  BoardManClipboardTypesPreferenceViewController(nibName: "BoardManClipboardTypesPreferenceViewController", bundle: nil),
                                  BoardManExcludedAppsPreferenceViewController(nibName: "BoardManExcludedAppsPreferenceViewController", bundle: nil),
                                  BoardManShortcutsPreferenceViewController(nibName: "BoardManShortcutsPreferenceViewController", bundle: nil),
                                  BoardManUpdatesPreferenceViewController(nibName: "BoardManUpdatesPreferenceViewController", bundle: nil),
                                  BoardManServicesPreferenceViewController(nibName: "BoardManServicesPreferenceViewController", bundle: nil)]
...
// MARK: - IBActions
extension BoardManPreferencesWindowController {
...
extension BoardManPreferencesWindowController: NSWindowDelegate {
    func windowWillClose(_ notification: Notification) {
        if let viewController = viewController[2] as? BoardManClipboardTypesPreferenceViewController {
...
// MARK: - Layout
private extension BoardManPreferencesWindowController {
    ...
    func makeAppearancePreferencesScaffold(frame: NSRect, useLiquidGlass: Bool) -> NSView {
        ...
        addBackgroundModes(to: left, originY: left.frame.height - 126)
        addSourcePath(to: left, originY: left.frame.height - 172)
        addAdjustmentSection(to: left, originY: left.frame.height - 376)
        addShapeSection(to: left, originY: left.frame.height - 505)
        addPresetSection(to: left, originY: 18)
        ...
        addPreviewScaffold(to: preview)
        addAdvancedAppearanceControl(to: preview)
        return root
    }

    func addBackgroundModes(to view: NSView, originY: CGFloat) {
        let modes = [
        ...
        for (index, mode) in modes.enumerated() {
            ...
            let card = appearanceTile(symbol: mode.0, title: mode.1, active: index == 1, locked: false)
            card.frame = NSRect(x: originX, y: originY, width: 86, height: 76)
            view.addSubview(card)
        }
    }

    func addSourcePath(to view: NSView, originY: CGFloat) {
        ...
        field.textColor = BoardManPreferenceUI.secondaryText
    ...
    func addAdjustmentSection(to view: NSView, originY: CGFloat) {
        ...
        for (index, control) in controls.enumerated() {
            let column = index % 2
            let row = index / 2
            addSliderRow(to: view, title: control.0, value: control.1, originX: 18 + CGFloat(column) * 280, originY: originY + 94 - CGFloat(row) * 34, locked: false)
        }
    }

    func addShapeSection(to view: NSView, originY: CGFloat) {
        ...
        for (index, control) in controls.enumerated() {
            let column = index % 2
            let row = index / 2
            addSliderRow(to: view, title: control.0, value: control.1, originX: 18 + CGFloat(column) * 280, originY: originY + 26 - CGFloat(row) * 34, locked: false)
        }
    }

    func addPresetSection(to view: NSView, originY: CGFloat) {
        ...
        for (index, preset) in presets.enumerated() {
            let tile = presetTile(title: preset.0, color: preset.1, active: index == 0, locked: false)
            tile.frame = NSRect(x: 18 + CGFloat(index) * 104, y: originY, width: 86, height: 76)
            view.addSubview(tile)
        }
    }

    func addPreviewScaffold(to view: NSView) {
        ...
        BoardManPreferenceUI.prepare(panel, color: BoardManPreferenceUI.base.withAlphaComponent(0.82), radius: 16, border: BoardManPreferenceUI.borderNormal)
    ...
    func addAdvancedAppearanceControl(to view: NSView) {
        let card = NSView(frame: NSRect(x: 46, y: view.frame.height - 166, width: view.frame.width - 92, height: 96))
        BoardManPreferenceUI.prepare(card, color: BoardManPreferenceUI.card, radius: BoardManPreferenceUI.Radius.card, border: BoardManPreferenceUI.borderNormal)

        let title = BoardManPreferenceUI.label("Appearance Style", size: 14, weight: .semibold)
        title.frame = NSRect(x: 18, y: 58, width: 180, height: 22)
        card.addSubview(title)

        let helper = BoardManPreferenceUI.label("Customize gradient, glass, blur, depth, and presets locally.", size: 12, color: BoardManPreferenceUI.secondaryText)
        helper.frame = NSRect(x: 18, y: 35, width: max(180, card.frame.width - 36), height: 18)
        card.addSubview(helper)

        let popup = NSPopUpButton(frame: NSRect(x: 18, y: 8, width: max(180, card.frame.width - 36), height: 26), pullsDown: false)
