...
struct AppEnvironment {
    ...
    static func fromStorage(defaults: UserDefaults = .standard) -> Environment {
        var excludeApplications = [BoardManApplicationInfo]()
        BoardManApplicationInfo.registerLegacyArchiveAliases()
        if let data = defaults.object(forKey: Constants.UserDefaults.excludeApplications) as? Data,
           let applications = NSKeyedUnarchiver.unarchiveObject(with: data) as? [BoardManApplicationInfo] {
