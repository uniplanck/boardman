...
extension PasteService {
    @discardableResult
    func paste(with clip: BoardManClip, shortcut: KeyCombo) -> Bool {
        guard !clip.isInvalidated else { return false }
        if NSKeyedUnarchiver.unarchiveObject(withFile: clip.dataPath) is BoardManClipData {
    ...
    @discardableResult
    func paste(with clip: BoardManClip) -> Bool {
        guard !clip.isInvalidated else { return false }
        guard let data = NSKeyedUnarchiver.unarchiveObject(withFile: clip.dataPath) as? BoardManClipData else {
    ...
    private func pasteRecoveredText(with clip: BoardManClip) -> Bool {
    ...
    func copyToPasteboard(with clip: BoardManClip) {
        lock.lock(); defer { lock.unlock() }

        guard let data = NSKeyedUnarchiver.unarchiveObject(withFile: clip.dataPath) as? BoardManClipData else { return }
...
extension PasteService {
    ...
    func paste() -> Bool {
        ...
        return sendShortcut(KeyCombo(key: .letterV, cocoaModifiers: .command))
    }
